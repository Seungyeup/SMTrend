...
def execute_statement(*, trino_statement_url: str, sql: str, user: str, catalog: str, schema: str) -> dict[str, Any]:
    headers = {
        "X-Trino-User": user,
...
def execute_sql_text(*, trino_statement_url: str, sql_text: str, user: str, catalog: str, schema: str) -> list[dict[str, Any]]:
...
def records_from_result(result: dict[str, Any]) -> list[dict[str, Any]]:
    columns = [column.get("name", f"col_{idx}") for idx, column in enumerate(result.get("columns", []))]
    rows = []
...
def _fetch_dates(*, table_name: str, trino_statement_url: str, user: str, catalog: str, schema: str) -> list[date]:
...
def main() -> None:
    ...
    parser.add_argument("--trino-statement-url", default=os.getenv("TRINO_STATEMENT_URL", "http://localhost:8080/v1/statement"))
    ...
    print(f"Incremental gold refresh completed: market_dates={len(market_dates)}, macro_dates={len(macro_dates)}")
