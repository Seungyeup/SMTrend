...
with DAG(
    ...
    validate_external = BashOperator(
        task_id="validate_external_tables",
        bash_command=(
            "cd /opt/project && "
            "python scripts/run_trino_sql_http.py "
            "--sql-file /opt/project/trino/sql/04_validate_external_tables.sql "
            "--require-rows "
            "--expect-first-value-ge 1"
        ),
    )
    ...
    validate_gold = BashOperator(
        ...
        bash_command=(
            "cd /opt/project && "
            "python scripts/run_trino_sql_http.py "
            "--sql-file /opt/project/trino/sql/05_validate_gold_tables.sql "
            "--require-rows "
            "--expect-first-value-ge 1"
        ),
    )

    create_external >> validate_external >> refresh_gold >> validate_gold
