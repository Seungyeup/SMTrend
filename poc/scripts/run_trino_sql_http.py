...
def main() -> None:
    ...
    parser.add_argument("--expect-first-value-ge", type=float)
    ...
    last_result = results[-1] if results else {"columns": [], "data": []}
    records = records_from_result(last_result)

    if args.require_rows and not records:
        raise RuntimeError("Trino query returned no rows")

    if args.expect_first_value_ge is not None:
        if not last_result.get("data"):
            raise RuntimeError("Trino query returned no data for numeric assertion")
        first_value = last_result["data"][0][0]
        numeric_value = float(first_value)
        if numeric_value < args.expect_first_value_ge:
            raise RuntimeError(
                f"First query value {numeric_value} is less than expected minimum {args.expect_first_value_ge}"
            )
