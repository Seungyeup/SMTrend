...
def test_build_market_trade_event_rejects_invalid_price() -> None:
    with pytest.raises(ValueError, match="price"):
        build_market_trade_event(
            source="finnhub_mock",
            symbol="GOOG",
            price=0.0,
            size=100,
...
def test_build_macro_release_event_rejects_invalid_date() -> None:
    with pytest.raises(ValueError, match="observation_date"):
        build_macro_release_event(
            source="fred_batch",
            series_id="DFF",
            observation_date="20260325",
            value=4.5,
            release_ts_ms=1_700_000_000_000,
            realtime_start="2026-03-25",
            realtime_end="2026-03-25",
